...
export default function UserBillsList() {
  ...
  const fetchUserBills = async () => {
    try {
      ...
      const response = await axios.get(
        `/api/admin/user-bills/${username}?page=${page}&limit=${rowsPerPage}`
      );
  ...
  return (
    <div className='space-y-6 p-4'>
      <div className='flex items-center justify-between'>
        <h1 className='text-2xl font-bold'>Hóa đơn của @{username}</h1>
        <Link
      ...
      <Card>
        <CardBody>
          <Table
            aria-label='Danh sách hóa đơn của người dùng'
            bottomContent={
              pages > 0 ? (
